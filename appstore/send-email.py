...
def with_retry(retry_config: RetryConfig) -> Callable[[Callable[P, T]], Callable[P, T]]:
...
class EmailConfig:
    ...
    def from_env(cls) -> "EmailConfig":
        ...
        required_vars = ["SMTP_SERVER", "SMTP_PORT", "EMAIL_SENDER", "EMAIL_RECEIVER", "SUBJECT", "BODY_FILE"]
        ...
        # Optional retry config from env
...
def build_message(config: EmailConfig) -> MIMEText:
    ...
    if not config.body_file.exists():
        raise FileNotFoundError(f"Body file not found: {config.body_file}")

    body = config.body_file.read_text(encoding="utf-8")
